...
export default {
    ...
    methods: {
        ...
        changeAction(file, fileList) {
            // console.log('69', file.name)
            //  for(let i in fileList){
            //     console.log('70', file.name, fileList[i].name)
            // }   
        },
        //上传过程（验证）
        processAction(event, file, fileList){
            ...
            // 多文件上传 
                // if(fileList.length > max) {
                    // debugger;
                    if(fileList.length > max) this.$message.error('上传文件不能超过' + max + '张');
                    ...
                    if(!supportTip && !fileMaxSize){ 
                        for(let i in fileList){
                            console.log('111)))', fileList[i].name)
                        }   
                        if(fileList.length > max) fileList.splice(0, fileList.length-max);
