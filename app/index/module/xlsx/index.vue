<template>
    <div class="index" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="拼命加载中...">
        ...
        <a id="downlink"></a>
        <el-button class="button" @click="uploadFile()">导入</el-button>*（自行添加xlsx文件测试，模拟数据在data）
    ...
    methods: {
        ...
        dealFile: function(data) {   // 处理导入的数据
            ...
            if (data.length <= 0) {
                ...
                this.excelData = data.map(one  =>  {
                    // debugger
                    // let o =  Object.values(one)
                    return {name: one['名称'],size: one['分量'], taste: one['口味'],price: one['单价(元)'], remain: one['剩余(份)']  }
                })
